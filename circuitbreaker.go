...
func (cb *FrequencyBreaker) Fail() int64 {
	if cb.Duration > 0 {
		cb.frequencyFail()
	}

	failures := cb.TrippableBreaker.Fail()
...
// TODO need a Successes() like Failures()
// TODO need a fequencySuccess()
...
func (cb *TimeoutBreaker) Fail() int64 {
	return 0
}
...
type RateBreaker struct {
	...
	_failureTick       unsafe.Pointer
	failuresSinceTick  int64
	_successTick       unsafe.Pointer
	successesSinceTick int64
	*TrippableBreaker
}
...
func (cb *RateBreaker) Success() {
	cb.frequencySuccess()
	cb.TrippableBreaker.Success()
}
...
func (cb *RateBreaker) frequencySuccess() {
	if time.Since(cb.successTick()) > cb.sampleRate {
		now := time.Now()
		atomic.StorePointer(&cb._successTick, unsafe.Pointer(&now))
		atomic.SwapInt64(&cb.successes, 0)
	}
}

func (cb *RateBreaker) successTick() time.Time {
	if cb._successTick == nil {
		now := time.Now()
		atomic.StorePointer(&cb._successTick, unsafe.Pointer(&now))
		return now
	}
	ptr := atomic.LoadPointer(&cb._successTick)
	return *(*time.Time)(ptr)
}
...
func (c *noOpBreaker) Fail() int64                     { return 0 }
func (c *noOpBreaker) Success() int64                  { return 0 }
func (c *noOpBreaker) Trip()                           {}
func (c *noOpBreaker) Reset()                          {}
func (c *noOpBreaker) Break()                          {}
func (c *noOpBreaker) Failures() int64                 { return 0 }
func (c *noOpBreaker) Successes() int64                { return 0 }
